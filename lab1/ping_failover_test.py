...
def main():
    ...
    def run_ping():
        ...
        out = conn.send_command(f"ping {target_ip} repeat 1 timeout 1 source {source_ip}")
        success = "!" in out
    ...
    print("Vlan mismatch on R3...")
    conn.send_config_set(["interface Gi0/0", "shutdown"])
    fail_time = datetime.datetime.now()
    log_rows.append(f"--- INTERFACE SHUTDOWN AT {fail_time.strftime('%H:%M:%S')} ---")
    debug_log.append(f"--- SHUTDOWN INITIATED AT {fail_time} ---")
    ...
    with open("vlan_debug_failover_r3.txt", "w") as f:
        f.write("\n".join(debug_log) + "\n")
