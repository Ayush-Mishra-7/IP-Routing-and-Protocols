#!/usr/bin/env python3
"""
Objective 4.1 automation.

This script performs the configuration changes for the mixed OSPF/RIP task:
1. Change OSPF area 5 back to a standard area by removing stub settings.
2. Remove OSPF entirely from R4.
3. Remove OSPF from the R2/R3 interface that faces R4.
4. Configure RIPv2 on R4 and on the R2/R3 shared segment toward R4.

The script intentionally does not configure redistribution between OSPF and RIP.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("objective_4_1.log", mode="a"),
    ],
)
...
def build_router_commands(router_name: str) -> list[str]:
    ...
    if router_name == "5R2":
        return [
            *ospf_cleanup,
            "router ospf 1",
            "no network 5.0.234.0 0.0.0.255 area 0",
            "no network 5.0.234.0 0.0.0.255 area 5",
            "exit",
            "no router rip",
            "router rip",
            "version 2",
    ...
    raise ValueError(f"unsupported router for this objective: {router_name}")
...
def main() -> None:
    ...
    target_routers = {"5R1", "5R2", "5R3", "5R4", "5R5"}
